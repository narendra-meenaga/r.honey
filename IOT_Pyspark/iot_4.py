...
inputFiles = ['normal1_short.csv','normal2_short.csv','bug1_short.csv']
...
# Performing Imputation - Fill missing values
dict_impute_values = {}
...
if imputeValue == "mean":
	...
	for column_name in merged_columns:
    	    ...
    	    dict_impute_values[column_name] = mean

	df_merged = df_merged.fillna(dict_impute_values)

# Replace 'null' values 0 
elif imputeValue == "zero":
       
        for column_name in merged_columns:
                dict_impute_values[column_name] = 0 

        df_merged = df_merged.fillna(dict_impute_values)


else :
      print("ErrorUndefined Imputation Value")
...
merged_columns_wo_response = merged_columns
...
#df_win = Window.partitionBy(merged_columns_wo_response)
...
rdd_labelled_data = df_merged.rdd.map(lambda l:LabeledPoint(l[0],[l[1:]]))
